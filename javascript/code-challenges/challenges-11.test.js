...
const count = (target, input) => {
  let counter = 0;
  input.map(arr => arr.forEach(num =>
    target === num ? counter++ : counter
  ));
  return counter;
};
...
const totalSum = (input) => {
  let sum = 0;
  input.map(arr => sum += arr.reduce((acc, curVal) => acc += curVal));
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(row => {
    return row.filter(num =>
      typeof num === 'number' && num % 5 === 0).map(num =>
        Math.pow(2, num));
  });
};
...
let findMaleAndFemale = (data) => {
  let newArr = [];
  data.map(char => char.gender === 'male' || char.gender === 'female' ?
    newArr.push(char.name) : '');
